...
addnote.addEventListener("click", function (e) {
  ...
  if (nts == null) {
    totalnotes = []; // data will be in string form
  } else {
    totalnotes = JSON.parse(nts); // json.parse converts string into javascript object but text should be in json form
  }
  let ntsob = {
    title: titl.value,
    body: noteBody.value,
  ...
  localStorage.setItem("notes", JSON.stringify(totalnotes)); // to send data in web browser data must be in string form
  titl.value = "";
  noteBody.value = "";
...
function showNotes() {
  let nts = localStorage.getItem("notes");
  if (nts === null) {  
    //Try changing the name of the arrays // === for strictly exact 
    totaln = [];
  } else {
